...
void writeVTU(const std::string& filename, const std::vector<Particle>& particle_arr, int n, double radius)
{
    ...
    for (int i = 0; i < n; i++)
        f << "        " << particle_arr[i].p_position[0] << " " << particle_arr[i].p_position[1] << " " << particle_arr[i].p_position[2] << "\n";
